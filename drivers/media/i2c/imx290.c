...
static int imx290_set_hmax(struct imx290 *imx290, u32 val)
{
	...
	if (!ret)
		ret = imx290_write_reg(imx290, IMX290_HMAX_HIGH,
				       (hmax >> 8) & 0xFF);
...
static int imx290_set_vmax(struct imx290 *imx290, u32 val)
{
	...
	ret = imx290_write_reg(imx290, IMX290_VMAX_LOW, vmax & 0xFF);
	if (!ret)
		ret = imx290_write_reg(imx290, IMX290_VMAX_LOW + 1,
				       (vmax >> 8) & 0xFF);
	if (!ret)
		ret = imx290_write_reg(imx290, IMX290_VMAX_LOW + 2,
				       (vmax >> 16) & 0xFF);
	if (ret)
