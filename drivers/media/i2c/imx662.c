...
/*
 * The exact value of pixel_rate is not described in the datasheet, and
 * HMAX is defined as being in units related to the input clock
 * (seemingly fixed at 74.25MHz and not the raw clock) rather than
 * referencing a pixel clock.
 * Using a multiplier of 3 with the 74.25MHz, and dividing
 * V4L2_CID_HBLANK by 3, results in the standard control behaviour.
 */
#define IMX662_INPUT_CLOCK			74250000LL
#define IMX662_PIXEL_RATE_SCALE			3
...
static int imx662_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_HBLANK:
		cci_write(imx662->regmap, IMX662_HMAX,
			  (ctrl->val + crop->width) / IMX662_PIXEL_RATE_SCALE,
			  &ret);
		break;
	case V4L2_CID_VBLANK:
...
static int imx662_ctrls_init(struct imx662 *imx662)
{
	...
	pixel_rate = IMX662_INPUT_CLOCK * IMX662_PIXEL_RATE_SCALE;
	v4l2_ctrl_new_std(&imx662->ctrls, NULL, V4L2_CID_PIXEL_RATE, pixel_rate,
