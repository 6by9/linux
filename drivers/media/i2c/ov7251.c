...
struct ov7251 {
	...
	const struct ov7251_fi_info *current_fi;
...
static const struct ov7251_fi_info ov7251_fi_info_data[] = {
	{
		.timeperframe = {
			.numerator = 100,
			.denominator = 3000
				  ...
				  struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	for (i = 0; i < ARRAY_SIZE(ov7251_fi_info_data); i++) {
		if (index-- == 0) {
			fie->interval = ov7251_fi_info_data[i].timeperframe;
			return 0;
		}
	}
...
static const struct ov7251_fi_info *
ov7251_find_interval_by_ival(struct ov7251 *ov7251, struct v4l2_fract *timeperframe)
{
	...
	for (i = 0; i < ARRAY_SIZE(ov7251_fi_info_data); i++) {
		...
		fps_tmp = avg_fps(&ov7251_fi_info_data[i].timeperframe);
				     ...
				     struct v4l2_subdev_frame_interval *fi)
{
	...
	mutex_lock(&ov7251->lock);
	fi->interval = ov7251->current_fi->timeperframe;
	mutex_unlock(&ov7251->lock);
...
static int ov7251_set_frame_interval(struct v4l2_subdev *subdev,
				     struct v4l2_subdev_frame_interval *fi)
{
	const struct ov7251_fi_info *new_fi;
	struct ov7251 *ov7251 = to_ov7251(subdev);
	int ret = 0;

	mutex_lock(&ov7251->lock);
	new_fi = ov7251_find_interval_by_ival(ov7251, &fi->interval);

	if (new_fi != ov7251->current_fi) {
		/* Setting VBLANK will also update exposure ranges */
		ret = __v4l2_ctrl_s_ctrl(ov7251->vblank,
					 new_fi->vts);
		if (ret < 0)
			goto exit;

		ov7251->current_fi = new_fi;
	}
...
static int ov7251_init_ctrls(struct ov7251 *ov7251)
{
	...
	vblank_min = ov7251->current_mode->vts_min - ov7251->current_mode->height;
	vblank_def = ov7251->current_mode->vts_def - ov7251->current_mode->height;
...
static int ov7251_probe(struct i2c_client *client)
{
	...
	ov7251->current_fi = &ov7251_fi_info_data[0];
