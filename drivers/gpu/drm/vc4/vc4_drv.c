...
static int __maybe_unused vc4_pm_suspend(struct device *dev)
{
	struct drm_device *drm = dev_get_drvdata(dev);
...
static int __maybe_unused vc4_pm_resume(struct device *dev)
{
	struct drm_device *drm = dev_get_drvdata(dev);
...
static const struct dev_pm_ops vc4_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(vc4_pm_suspend, vc4_pm_resume)
};
